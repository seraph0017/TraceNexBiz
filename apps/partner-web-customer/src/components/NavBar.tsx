...
const LINKS: { to: string; key: string }[] = [
  ...
  { to: "/topup", key: "nav.topup" },
  { to: "/api-keys", key: "nav.api_keys" },
  { to: "/usage", key: "nav.usage" },
  { to: "/invoice", key: "nav.invoice" },
  { to: "/tickets", key: "nav.tickets" },
  { to: "/kyc", key: "nav.kyc" },
  { to: "/switch-partner", key: "nav.switch_partner" },
  { to: "/pipl-rights", key: "nav.pipl_rights" },
...
export function NavBar(): JSX.Element {
  ...
  return (
    ...
    >
      <strong style={{ fontSize: 16 }}>TraceNex</strong>
      ...
      >
        <span style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
          <Avatar size="small" alt="me">
            {me?.display_name?.slice(0, 1) ?? "C"}
          </Avatar>
          <span style={{ fontSize: 13, color: "#374151" }}>{me?.display_name ?? "—"}</span>
        </span>
      </Dropdown>
    </header>
  );
