...
describe("useThrottledSubmit (admin)", () => {
  ...
  it("非 Error 异常包装成 Error", async () => {
    ...
    const { result } = renderHook(() => useThrottledSubmit(fn, { coolDownMs: 1 }));
    await act(async () => {
      await expect(result.current.submit()).rejects.toThrow("string-err");
    });
